...
- The approved map encoding is dynamic-width, exactly 30 image rows, two logical 15-row layers (objects plus actors), and `TILE_SIZE=32`. World gameplay height derives from one 15-row logical layer, never from all 30 encoding rows and never from window height.
...
- Map variants are parsed but discarded at construction. CloudPlatform currently creates P2 `MovingBlock`, while P4 `CloudBlock` is separately required; `DEC-CLOUD-SEMANTICS` stays open.
...
| 8 | `P2-PIT-001` | P0 | `BLOCKED` | `BLOCKED_BY_OTHER_TASK` | `P2-EXTENT-001`, `DEC-MULTIPLAYER-DEATH`, `DEC-RESPAWN`, `CON-P1-P2-DEATH` |
...
|---|---|---|---|
| Stable non-owning `PlayerManager*` collection and selected world/level | Session owner follows open `DEC-PLAYER-OWNER` (P1 recommended); P3 implements player | Players outlive active P2 level and are injected before load/update | `P2-LOAD-001`, bounds, pit, contacts, hazards, completion |
| `DEC-ASSET-ROOT` | Human | Must be `OPEN` until recorded; resolved before load-path implementation | `P2-LOAD-001`, `P2-NINE-LEVEL-001` |
| `DEC-MULTIPLAYER-DEATH`, `DEC-RESPAWN` | Human | Stay `OPEN`; consumed before final pit/death integration | `P2-PIT-001` |
| `DEC-BLOCK-ACTOR-ELIGIBILITY`, `DEC-CLOUD-SEMANTICS` | Human | Stay `OPEN`; consumed before construction/contact behavior is frozen | `P2-VARIANT-WIRE-001`; enemy-driven block work if later required |
| `DEC-WINFLAG-POLISH` | Human | Stay `OPEN`; does not block base-anchor correction | Optional portion of `P2-WINFLAG-001` |
...
| P4 Block lifecycle/activation, variant constructors, spawn callbacks, and 32x32 visual-size invariant | P4 | P2 owns Block objects and immediately adopts callback spawns | physics, variants, environment, nine-level tasks |
...
| `CON-P1-P2-WORLD-EXTENT` | Provider | Immutable pixel extent derived after successful parse from dynamic width × 32 and one logical 15-row world layer × 32. Invalid before successful load; expose validity rather than a plausible zero rectangle. | MISSING; `P2-EXTENT-001`, bounds, pit |
...
| `CON-P2-P3-PLAYER-ITEM` | Orchestrator/consumer | P2 detects overlap after movement/before cleanup and supplies safe-growth clearance for proposed 32×64 bounds; P3 returns outcome; denied growth leaves player/item unchanged and inactive items cannot collect twice. | MISSING; contact-item |
...
| `CON-P2-P4-BLOCK-VISUAL-SIZE` | Construction consumer | P2 calls the P4-owned invariant that sets a 32×32 hitbox and scales 16×16 art exactly once before first render. | MISSING; variant/nine-level; P4 provider task |

## 8. Per-task implementation notes

The following are persistent task cards. Requirement labels are `REQUIRED_BY_PLAN`, `REQUIRED_BY_TEAM`, or `IMPLIED_BY_CURRENT_GROUP4_DESIGN`. Human decisions named here remain `OPEN` until the human decision register records a resolution.
...
- **Exact current behavior:** `MapManager` records dynamic tile width and validates a 30-row image with two 15-row logical layers at `TILE_SIZE=32`; no explicit pixel width/height value or readiness-aware extent exists, and `LevelManager` does not consume the width.
- **Missing checklist:** [ ] define a value extent API; [ ] derive width from loaded tile width; [ ] derive world height from one 15-row gameplay layer, not 30 encoding rows; [ ] expose only after successful load; [ ] keep units/naming explicit; [ ] add dynamic-width and invalid-load checks.
...
- **Dependencies:** existing parser validation. **Provides:** `CON-P1-P2-WORLD-EXTENT`, extent half of `CON-P1-P2-CAMERA`. **Consumes:** successful-map-load state and `TILE_SIZE=32`.
- **Group5 reference / do-not-copy:** Adapt the idea of map-derived bounds only. Do not copy Group5's 64-pixel/three-layer formula or raw owner graph.
...
- **Checks:** compile affected units with C++20/SFML 3.1; runtime assert two synthetic widths yield different exact pixel widths and logical height 15×32; visual N/A; gameplay verify P1 later clamps camera against the value.
...
- **Owner / priority / requirement / status:** Person 2; P0; `REQUIRED_BY_PLAN`; `READY`.
- **Purpose:** Ensure Lava reports death only when the player actually reaches the agreed fatal state, once.
- **Exact current behavior:** overlap calls `takeDamage(getHealth())`; a big/fire player can downgrade and remain alive, yet the LevelManager death callback fires unconditionally. No focused test proves once-only delivery.
- **Missing checklist:** [ ] define fatal outcome from P3's existing health/state API; [ ] call death only on actual fatal state; [ ] suppress repeated callbacks; [ ] stop further same-frame interactions for a dead player; [ ] preserve nonfatal downgrade if that is P3 behavior; [ ] test small and powered players.
...
- **Owner / priority / requirement / status:** Person 2; P0; `REQUIRED_BY_PLAN` plus `IMPLIED_BY_CURRENT_GROUP4_DESIGN`; `READY`.
- **Purpose:** Interpret the team's white marker as the flag base/activation anchor and preserve one-shot completion.
- **Exact current behavior:** parser creates `WinFlag` at `32*a,32*b`; WinFlag treats that point as the top of a 32×128 pole and LevelManager emits immediate completion once. Group5 trace proves the referenced white marker is a base/anchor, not the pole top.
- **Missing checklist:** [ ] keep marker world position as base/anchor; [ ] translate collision/visual geometry upward consistently; [ ] preserve one-shot activation/callback; [ ] make completion impossible on unready levels; [ ] test anchor math; [ ] leave delayed slide/polish unresolved under `DEC-WINFLAG-POLISH`.
...
- **Dependencies:** `CON-P1-P2-COMPLETION`; `DEC-WINFLAG-POLISH` is OPEN but nonblocking for anchor correctness. **Provides:** base-anchor activation and completion event. **Consumes:** parser world coordinate.
- **Group5 reference / do-not-copy:** Reuse base-anchor semantics only. Do not copy 64-pixel sizes, 470-pixel offsets, animation timing, raw pointers, or mediator design.
- **Notes / SFML:** use SFML 3.1 shapes/rect APIs already present; geometry must follow `TILE_SIZE=32` and not a magic map-specific coordinate.
...
- **Owner / priority / requirement / status:** Person 2; P1; `REQUIRED_BY_PLAN`; `READY`.
...
- **Exact current behavior:** Pipe is a two-tile-wide variable-height collider; Trampoline provides compression/launch behavior; MovingBlock supports horizontal/vertical travel and carries actors. No focused current tests or production-state verification exist. CloudPlatform mapping is separately unresolved.
- **Missing checklist:** [ ] direct Pipe dimensions/collision check; [ ] Trampoline launch and no double impulse check; [ ] horizontal and vertical MovingBlock travel limits; [ ] carry delta check; [ ] lifecycle/view registration check; [ ] document any tuning ceiling without map-content inspection.
...
- **Checks:** compile all exact targets; runtime assertions above; visual confirm primitive/collider alignment at 32-pixel tiles; gameplay verify player can stand on Pipe, launch once, and ride both platform directions.
- **Completion dimensions:** `SOURCE_DONE=YES`; `INTEGRATION_DONE=NO`; `RUNTIME_TESTED=NO`; `VISUALLY_VERIFIED=NO`; `GAMEPLAY_VERIFIED=NO`.
...
- **Exact current behavior:** Cannon creates `unique_ptr<Bullet>` through the pending-spawn path; Bullet motion/lifetime/removal is tested. Rocket resolves only a target position and safely handles target loss; it has no map enum or production spawner. Bullet/Rocket lack complete world collision/culling.
- **Missing checklist:** [ ] define a minimal production Rocket spawn entry point; [ ] accept P3 boss/fire attack requests under `CON-P2-P3-PROJECTILE-SPAWN`; [ ] keep immediate unique ownership; [ ] collide/cull Bullet and Rocket against agreed world geometry/extent; [ ] deliver player damage/death once; [ ] remove on hit, expiry, target loss, or internal P2 off-world policy; [ ] test birth/update-order edge cases.
...
- **Completion dimensions:** `SOURCE_DONE=NO`; `INTEGRATION_DONE=NO`; `RUNTIME_TESTED=PARTIAL`; `VISUALLY_VERIFIED=NO`; `GAMEPLAY_VERIFIED=NO`.
...
- **Exact current behavior:** nine wrappers select world/level through `ConfiguredLevel`; its path points at nonexistent `assets/levels`, the named PNGs currently sit under `assets/textures`, `LevelManager::load` returns a Boolean and parser error text, but constructor discards the result.
- **Missing checklist:** [ ] resolve `DEC-ASSET-ROOT`; [ ] build one coherent path without per-wrapper duplication; [ ] store/expose ready state and diagnostic; [ ] make update/render safe when unready; [ ] preserve selected players across valid load/reload only; [ ] test valid/missing/invalid paths without editing images.
...
- **Dependencies:** OPEN `DEC-ASSET-ROOT`, accepted `CON-P1-P2-LEVEL-LOAD`, `CON-P2-P3-ENTITY-CONSTRUCTION`. **Provides:** readiness/diagnostic to P1 and nine wrappers. **Consumes:** a stable player collection from the owner approved by `DEC-PLAYER-OWNER`, P3 factory result, and the chosen runtime asset root.
...
- **Owner / priority / requirement / status:** Person 2; P0; `IMPLIED_BY_CURRENT_GROUP4_DESIGN`; `BLOCKED`.
...
- **Dependencies:** `P2-EXTENT-001`, `CON-P1-P2-WORLD-EXTENT`, `CON-P1-P2-CAMERA`. **Provides:** bounded player world position. **Consumes:** hitbox and movement velocity.
...
- **Exact current behavior:** gravity allows falling indefinitely; there is no below-world check. A death callback exists but only selected hazards call it.
- **Missing checklist:** [ ] consume valid logical-world bottom; [ ] define one-tile cleanup/death margin in contract; [ ] test after movement/physics; [ ] latch once per affected player/attempt; [ ] exclude the fallen player from later frame interactions; [ ] clear latch only on agreed reload/reset; [ ] never mutate lives or states directly.
...
- **Dependencies:** `P2-EXTENT-001`; OPEN `DEC-MULTIPLAYER-DEATH`, `DEC-RESPAWN`; `CON-P1-P2-DEATH`, `CON-P1-P2-ACTIVE-LEVEL`. **Provides:** affected-player fatal event. **Consumes:** borrowed player and valid extent.
...
- **Notes / SFML:** pure runtime logic. Two-player behavior remains deliberately blocked until decisions are recorded.
...
- **Exact current behavior:** inactive specialized objects are removed after traversal and views rebuild; no general map-derived off-world policy exists. BrickFragment expires at hard-coded y=1080 in P4 code and must not be reassigned to P2.
- **Missing checklist:** [ ] define margins by object category; [ ] use extent, not window; [ ] mark through owned type's lifecycle API; [ ] remove only after traversal; [ ] rebuild all affected views; [ ] preserve valid temporarily airborne objects; [ ] cover left/right/below exits.
...
- **Checks:** compile; runtime Goomba stomp/side, Heriss harmful stomp, Koopa shell transitions, invulnerability, two players, inactive removal/order; visual verify bounce/contact alignment; gameplay verify score/death handoff once.
- **Completion dimensions:** `SOURCE_DONE=NO`; `INTEGRATION_DONE=NO`; `RUNTIME_TESTED=NO`; `VISUALLY_VERIFIED=NO`; `GAMEPLAY_VERIFIED=NO`.
...
- **Missing checklist:** [ ] active player/item overlap after movement; [ ] P3 typed collection outcome; [ ] one-shot deactivation; [ ] Mushroom/block world physics where contract requires; [ ] queue-safe removal; [ ] outcome forwarded to P1/P4 via owner contract; [ ] two-player simultaneous-overlap rule.
- **Exact target files:** `include/Physics/PhysicsEngine.hpp`, `src/Physics/PhysicsEngine.cpp`, `include/Levels/Managers/LevelManager.hpp`, `src/Levels/Managers/LevelManager.cpp`, `tests/Person2RuntimeContracts.cpp`.
...
- **Notes / SFML:** use hitboxes, not sprite bounds. Contract must specify the winner for simultaneous 2P overlap before gameplay verification.
- **Checks:** compile; runtime Coin/Mushroom/power-up exactly once, two-player overlap, inactive item skipped, spawned-during-frame pickup deferred consistently; visual verify item contact alignment; gameplay verify score/state/HUD effects after cross-owner integration.
- **Completion dimensions:** `SOURCE_DONE=NO`; `INTEGRATION_DONE=NO`; `RUNTIME_TESTED=NO`; `VISUALLY_VERIFIED=NO`; `GAMEPLAY_VERIFIED=NO`.
...
- **Missing checklist:** [ ] resolve Cloud marker semantics; [ ] accept P4 constructor/factory surface; [ ] pass Brick/MushroomBlock variants unchanged; [ ] connect P3 payload type where specified; [ ] retain immediate spawn adoption; [ ] diagnose unsupported variants; [ ] call P4 32×32 invariant.
...
- **Dependencies:** OPEN `DEC-CLOUD-SEMANTICS`, `DEC-BLOCK-ACTOR-ELIGIBILITY`; `CON-P2-P4-BLOCK-VARIANTS`, `CON-P2-P4-SPAWN-HANDOFF`, `CON-P2-P4-BLOCK-VISUAL-SIZE`; P4/P3 providers.
...
- **Checks:** compile; runtime each supported variant produces the expected dynamic type/config and callback ownership; visual verify 32×32 variant art; gameplay verify representative payload and distinct Cloud/MovingBlock behavior; no PNG inspection/edit in this task.
- **Completion dimensions:** `SOURCE_DONE=NO`; `INTEGRATION_DONE=NO`; `RUNTIME_TESTED=NO`; `VISUALLY_VERIFIED=NO`; `GAMEPLAY_VERIFIED=NO`.
...
- **Exact current behavior:** all nine wrapper header/source pairs exist and select world/level generically, but wrong path/readiness, extent/bounds/pit, interactions, variants, visuals, and production state flow prevent meaningful play validation.
- **Missing checklist:** [ ] shared load path ready; [ ] no wrapper-specific engine duplication; [ ] each exact wrapper identity maps to its exact filename; [ ] extent/bounds/pit valid; [ ] representative contacts/variants/hazards valid; [ ] P1 owns active-level transitions while selected-player ownership follows approved `DEC-PLAYER-OWNER`; [ ] record nine separate validation results; [ ] map issues stay user-owned.
...
- **Notes / SFML:** all nine use C++20/SFML 3.1 and 32-pixel/two-layer architecture. A wrapper may contain only identity/configuration; shared bugs are fixed once in managers.
...
- **Completion dimensions:** `SOURCE_DONE=PARTIAL`; `INTEGRATION_DONE=NO`; `RUNTIME_TESTED=NO`; `VISUALLY_VERIFIED=NO`; `GAMEPLAY_VERIFIED=NO`.
...
|---|---|---|---|
| `P2-EXTENT-001` | Valid/invalid readiness; two dynamic widths; 15×32 logical height | N/A | P1 camera consumes exact value |
...
| `P2-PIT-001` | Threshold boundaries, duplicate suppression, chosen 2P identity | N/A | Agreed death/respawn policy |
| `P2-CLEANUP-001` | Each owner category off-world; view rebuild; legal airborne retention | N/A | Stable counts in extended session |
| `P2-CONTACT-ENEMY-001` | Goomba/Heriss/Koopa, invulnerability, 2P, removal ordering | Contact/bounce alignment | Score/death outcomes once |
| `P2-CONTACT-ITEM-001` | Coin/Mushroom/power-up, 2P race, spawned-frame order | Pickup alignment | State/score/HUD update once |
| `P2-VARIANT-WIRE-001` | Dynamic type/config per variant; unsupported diagnostic; ownership | 32×32 correct variant art | Representative payload and Cloud policy |
...
| `LV-W3-LV3` | `W3_LV3` | same, separately recorded | same, separately recorded | production render | full final-level completion path | `BLOCKED` |

### Cross-owner vertical-slice order

1. P1 selects 1–2 P3 players; the owner approved by `DEC-PLAYER-OWNER` retains them (P1 recommended), and P1 owns the P2 active level (`CON-P1-P2-ACTIVE-LEVEL`).
...
- `MapManager.hpp/.cpp` changes must preserve exact 30-row/two-layer validation, dynamic width, variants, diagnostics, and `TILE_SIZE=32`.
...
- World extent is derived from dynamic width, one 15-row logical gameplay layer, and `TILE_SIZE=32`; bounds, pit death, cleanup, and P1 camera consume that same value.
