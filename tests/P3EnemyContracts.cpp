...
int main() {
    ...
    auto koopaRes2 = koopa.handlePlayerContact(player, 2, 1.f); // Side hit -> Kick
    checkP3(koopaRes2.result == EnemyContactResult::ShellKicked && koopa.isShellKicked(), "Koopa shell kicked");
    
    // Heriss
    Heriss heriss({0.f, 0.f});
    auto herissRes = heriss.handlePlayerContact(player, 1, 0.f); // Stomp
    ...
    checkP3(proj.has_value() && proj->type == "BowserFire" && !bowser.hasPendingProjectile(), "Bowser projectile consumed");
